...
func TestLeaderFailure2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	// log.Printf("leader1 is: %v", leader1)
...
func TestFailAgree2B(t *testing.T) {
	...
	// log.Println("disconnecting server ", (leader+1)%servers)
	...
	// log.Println("reconnecting server ", (leader+1)%servers)
...
func TestRejoin2B(t *testing.T) {
	...
	// log.Println("sent three starts")

	// new leader commits, also for index=2
	cfg.one(103, 2, true)
	// log.Println("committed 103")

	// new leader network failure
	leader2 := cfg.checkOneLeader()
	// log.Println("leader2 is ", leader2)
	cfg.disconnect(leader2)
	// log.Println("disconnected leader2")

	// old leader connected again
	cfg.connect(leader1)
	// log.Println("connecting leader1: ", leader1)

	cfg.one(104, 2, true)
	// log.Println("committed 104")

	// all together now
	cfg.connect(leader2)
	// log.Println("connecting leader2: ", leader2)

	cfg.one(105, servers, true)
	// log.Println("committed 105")
...
func TestBackup2B(t *testing.T) {
	...
	// log.Println("disconnected ", (leader1+2)%servers)
	cfg.disconnect((leader1 + 3) % servers)
	// log.Println("disconnected ", (leader1+3)%servers)
	cfg.disconnect((leader1 + 4) % servers)
	// log.Println("disconnected ", (leader1+4)%servers)
	...
	// log.Println("disconnected ", (leader1+0)%servers)
	cfg.disconnect((leader1 + 1) % servers)
	// log.Println("disconnected ", (leader1+1)%servers)

	// allow other partition to recover
	cfg.connect((leader1 + 2) % servers)
	// log.Println("connected ", (leader1+2)%servers)
	cfg.connect((leader1 + 3) % servers)
	// log.Println("connected ", (leader1+3)%servers)
	cfg.connect((leader1 + 4) % servers)
	// log.Println("connected ", (leader1+4)%servers)

	// lots of successful commands to new group.
	for i := 0; i < 50; i++ {
		cfg.one(rand.Int(), 3, true)
	}
	// log.Println("Lots of successful commands to new group (1)")
	...
	// log.Println("disconnected ", other)
	...
	// log.Println("connected ", (leader1+0)%servers)
	cfg.connect((leader1 + 1) % servers)
	// log.Println("connected ", (leader1+1)%servers)
	cfg.connect(other)
	// log.Println("connected ", other)
	// log.Println("Original leaders brought back to life")

	// lots of successful commands to new group.
	for i := 0; i < 50; i++ {
		cfg.one(rand.Int(), 3, true)
	}
	// log.Println("Lots of successful commands to new group (2)")
	...
	// log.Println("Everyone connected!")
	cfg.one(rand.Int(), servers, true)
...
func TestPersist12C(t *testing.T) {
	...
	cfg.end()
	// log.Println("TestPersist12C done")
	// time.Sleep(3 * time.Second)
}

func TestPersist22C(t *testing.T) {
	// sleep for 2 seconds
	// log.Println("TestPersist22C start")
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		// log.Println("nn is ", nn)
		for i := 0; i < nn; i++ {
			cfg.rafts[sender].Start(rand.Int())
		}
		// log.Println("all nn sent")
